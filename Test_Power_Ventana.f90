program simanhosc
    ...
    unit_summary = 20
    open(unit=unit_summary, file="Summary_Evolution_Cosmic_Perturbations_N_0_23.dat", status="unknown", action="write", form="formatted")

    ! Bucle para iterar sobre los valores de N_ref
    do iter = 1, 3   !100
        N_ref = 0.5	 * iter   !0.6 * iter

        ! Configuración inicial
        yinit(1) = 25.0  ! Campo escalar inicial 30
        yinit(2) = 0.0  ! Derivada de φ inicial
        yinit(3) = hubb(yinit)  ! Cálculo inicial de Hubble H
        yinit(4) = -5.0  ! Establecer ln(a) inicial a N_ref
        ...
        if (elipse) then
            write(filename, '("Evolution_Cosmic_Perturbations_18_open_2_N_ref_0_", I1, ".dat")') iter
...
subroutine gl8(y, dt, kcom)
    ...
    real y(n), kcom, g(n,s), dt; integer i, k
